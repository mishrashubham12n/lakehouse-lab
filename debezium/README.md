...
| ID | Module | Status |
|----|--------|--------|
| `CDC-1` | [Local Postgres & logical replication](cdc1_logical_replication.ipynb) — `wal_level=logical`, publication, replication slot; WAL the slot pins | `[x]` |
| `CDC-2` | [Debezium connector bring-up](cdc2_connector_bringup.ipynb) — register via the Connect REST API; initial snapshot then streaming | `[x]` |
| `CDC-3` | [Snapshot vs streaming phases](cdc3_snapshot_modes.ipynb) — `snapshot.mode` (`initial` vs `never`); the restart-from-scratch caveat | `[x]` |
| `CDC-4` | [The CDC event envelope](cdc4_event_envelope.ipynb) — `before`/`after`/`op`/`ts_ms`/`source`; flatten with `ExtractNewRecordState` | `[x]` |
| `CDC-5` | [Replication slot & WAL growth ⚠️](cdc5_wal_growth.ipynb) — inactive slot retains WAL → disk grows; detect via `pg_replication_slots`; `max_slot_wal_keep_size` | `[x]` |
| `CDC-6` | [Tombstones, deletes & replica identity](cdc6_deletes.ipynb) — `op=d` + tombstone; `REPLICA IDENTITY FULL` for the full `before` | `[x]` |
| `CDC-7` | [CDC → Spark → Iceberg upsert pipeline](cdc7_upsert_pipeline.ipynb) — `MERGE` c/u/d into an Iceberg mirror; idempotent by LSN | `[x]` |
| `CDC-8` | [CDC schema evolution](cdc8_schema_evolution.ipynb) — `ALTER TABLE` upstream; no DDL events; evolve the Iceberg sink (LAK-6) | `[x]` |
| `CDC-9` | [Deep failure-mode tour](cdc9_failure_modes.ipynb) — pause/resume/restart & offset recovery; ordering; effectively-once reasoning | `[x]` |

## Layout

Flat — one `cdc<N>_<topic>.ipynb` per module (paths in the table above). All built and
**live-verified** end-to-end against the Postgres + Kafka Connect + Spark + Iceberg stack.
