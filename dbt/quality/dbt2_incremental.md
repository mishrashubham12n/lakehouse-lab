...
[LAK-8](../../iceberg/lak8_merge.ipynb) — MOR defers the rewrite by writing delete files instead.
...
- **`merge` is copy-on-write under the hood** — touching one row can rewrite a whole partition. On
  big tables, weigh COW vs merge-on-read ([LAK-8](../../iceberg/lak8_merge.ipynb)) and watch the write
