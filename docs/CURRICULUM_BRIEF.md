# lakehouse-lab — Curriculum Brief (Master Spec)

> This is the **why / what / rules** document for evolving `lakehouse-lab` into a complete,
...
Turn `lakehouse-lab` from a working batch + streaming + dbt + Airflow skeleton into a
...
The curriculum spans every tool in the repo: **Spark, the three OSS lakehouse formats
(Iceberg / Delta / Hudi), the catalog layer under them (`spark_catalog`, `iceberg_catalog` on Hadoop,
`nessie_catalog` on Nessie native `/api/v2`, `polaris_catalog` read-only + RBAC, `glue_catalog` on
MiniStack), Kafka, Debezium (CDC), dbt, Airflow**, plus a data-quality layer (**dbt tests + Great
Expectations**) and an optional observability layer (Prometheus + Grafana, opt-in via
`make monitoring-up`).
...
- **Build on the existing architecture** (unified Spark Thrift+Connect server on Spark 4.0.2;
  Iceberg 1.10.1 / Delta 4.0.0 / Hudi 1.2.0 all baked in; five registered catalogs on
  `s3a://warehouse/…` via MiniStack; Kafka in KRaft; dbt-via-thrift; Airflow 3). Extend;
  don't re-platform.
